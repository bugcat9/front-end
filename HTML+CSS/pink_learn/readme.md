...
### 第8天
```
相对定位 relative相对于原来位置移动，元素设置此属性之后仍然处在文档流中，不影响其他元素的布局
绝对定位 absolute元素会脱离文档流，如果设置偏移量，会影响其他元素的位置定位
    在父级无定位时：元素相对于根元素定位（即html元素）
    在父级有定位时：元素会对于离自己最近的设置了相对或绝对定位的父元素进行定位
...
字体图标
vertical-align 对齐
text-overflow 实现单行文字显示省略号

```
### 第10天
```
HTML5 新增语义化标签 header nav section
HTML5新增视频标签 video
HTML5新增音频标签 audio
CSS3新增结构伪类选择器 first-child last-child nth-child(2)
伪元素选择器before和after
```
### 第11天+第12天
```
